...
function authenticate () {
  ...
  const stubbedResponse = '{"access_token":"SOMETOKEN","token_type":"bearer","expires_in":3600}'
...
function searchJournals (token, term) {
  const stubbedResponse = '{ "total": 3, "start": 0, "entries": [{ "link": "https://libcat.uncw.edu/iii/sierra-api/v5/bibs/1000953" }, { "link": "https://libcat.uncw.edu/iii/sierra-api/v5/bibs/1066502" }, { "link": "https://libcat.uncw.edu/iii/sierra-api/v5/bibs/1074149" }]}'
...
function getBibRecord (token, id) {
  const stubbedResponse = '{"1074149\": "null", "title": "Higher education opportunities for minorities and women, annotated selections"}'
  return JSON.parse(stubbedResponse)
  ...
  return new Promise((resolve, reject) => {
    request.get(options, (err, res, body) => {
      if (err) reject(err)
      return parseBib(body)
    })
  })
}

const parseBib = function (body) {
